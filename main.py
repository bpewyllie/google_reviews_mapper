...
if __name__ == "__main__":
    ...
    print(f"\n{len(county_locations)} gridpoints within county found\n")
    # city_locations = gps.tools.filter_grid_by_region(locations, city_gdf)
    gps.tools.map_grid_locs_static(
        county_locations,
        county_gdf,
        "SL County Locations",
    ...
    i = 0
    for location in county_locs:
        i += 1
        print(f"\nretrieving locations around gridpoint {i} of {len(county_locs)}", end="\r")
        results.extend(
